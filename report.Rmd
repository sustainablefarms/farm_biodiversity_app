...
imagetexfun <- function(specinfo){
  imagetex <- if (file.exists(specinfo$imgfilename)){
        paste0("\\includegraphics[width=1\\linewidth, height = 1\\linewidth, keepaspectratio]{",
...
buildspeclatex_justimage <- function(specname){
  ...
  imagetex <- imagetexfun(speciesinfo[specinforow,])
  out <- c(
    "\\begin{minipage}[b][0.30\\textwidth][t]{0.23\\textwidth}",
    "\\begin{center}",
    # "\\\\ \\vspace*{1ex} ",
    specname,
    "\\\\ ",
...
texs <- lapply(consstatus$CommonName, function(specname){
  heading <- paste0("\\subsection*{", specname, "}")
  intro <- paste("The", specname, consstatus[specname, "statussummary"],
          onespecwords(specname, data$species_prob_current))
  imagewstory <- buildspeclatex_imagestory(specname)
  return(c(heading, intro, imagewstory))
})
lapply(texs, cat)
```
