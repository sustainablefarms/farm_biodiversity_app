...
if (params$loadexampledata){
  ...
  data <- readRDS("predictions.rds")
  refpredictions <- readRDS("refpredictions.rds")
  refisaverage <- TRUE
} else {
  cpred <- cpred
  current_values <- cval
...
predcontext <- predictions_morecontext(cpred$species_prob_current, rpred$spec_prob, refisaverage)
...
topten <- order(cpred$species_prob_current[, "median"], decreasing = TRUE)[1:10]
...
for (specname in row.names(cpred$species_prob_current)[topten]){
...
bottomten <- order(cpred$species_prob_current[, "median"], decreasing = FALSE)[1:10]
...
cat(" \\begin{center} ")
for (specname in row.names(cpred$species_prob_current)[bottomten]){
...
texs <- lapply(consstatus$CommonName, function(specname){
  probs <- paste("The", specname, consstatus[specname, "statussummary"],
          onespecwords(specname, cpred$species_prob_current,
...
all_prob(tocommon(cpred$species_prob_current)) %>%
...
if (refisaverage){
  ...
  refRegion <- refval$selected_region
...
if (refisaverage){
  ...
  refpatchprops <- makeniceprops(t(newXocc_fromselected(refval)))
}
...
Below presents estimates of the average number of species occupying the `r if (nrow(current_values$patchattr_tbl) > 1){"patches"} else {"patch"}` specified `Your Estimate'.
Also shown are estimates of the average number of species if `r if (nrow(current_values$patchattr_tbl) > 1){"all patches have"} else {"the patch has"}` woody cover within 500m equal to 2% ('Less woody canopy nearby') or equal to 20% ('More woody canopy nearby').
The estimated average number of species in the reference scenario ('Reference') is also shown.

Due to the species in `r appname`, these estimates should be viewed as an estimate of the average number of sedentary, common, and land-based bird species occupying the `r if (nrow(current_values$patchattr_tbl) > 1){"patches"} else {"patch"}`.
