...
if (params$loadexampledata){
  ...
  data <- isolate(reactiveValuesToList(datar()))
  refpredictions <- isolate(refpredictions())
  current_values <- isolate(current_values())
  refisaverage <- isolate(refisaverage())
}
...
all_prob(tocommon(data$species_prob_current)) %>%
...
all_prob(tocommon(refpredictions)) %>%
              order_y(.data[["Body Length"]]) %>%
  add_error() %>%
  add_label_onright() 
```
...
richness_plot(data$species_richness, labelnudge = -0.25)
```
\end{center}
