...
if (params$loadexampledata){
  pkgload::load_all(".")
  main_app_prep()
  current_values <- readRDS("data/test-current_values_2patches.rds")
  data <- readRDS("data/test-predictions_data_2patches.rds")
} else {
...
Below presents estimates of the average number of species occupying the `r if (current_values$patches > 1){"patches"} else {"patch"}` specified `Your Estimate'.
Also shown are estimates of the average number of species if `r if (current_values$patches > 1){"all patches have"} else {"the patch has"}` woody cover within 500m equal to 2% ('Less woody canopy nearby') or equal to 20% ('More woody canopy nearby').
The estimated average number of species in the reference scenario ('Reference') is also shown.

Due to the species in `r appname`, these estimates should be viewed as an estimate of the average number of sedentary, common, and land-based bird species occupying the `r if (current_values$patches > 1){"patches"} else {"patch"}`.
...
\begin{center}
```{r specrich,  fig.height = 2, fig.width = 8, out.width = "80%"}
plot <- richness_plot(data$species_richness)
plot
